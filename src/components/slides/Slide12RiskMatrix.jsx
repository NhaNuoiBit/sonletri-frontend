...
export default function Slide12RiskMatrix({ index, total }) {
  ...
  return (
    <SlideShell index={index} total={total}>
      ...
      <p className="mt-2 text-slate-500">Phân tích chi tiết và mức độ ưu tiên xử lý</p>

      <div className="mt-4 grid lg:grid-cols-2 gap-6 flex-1 min-h-0">
          ...
          {items.map((b, i) => (
            ...
            >
              ...
              <div className="px-5 pb-4">
                <div className="rounded-xl bg-brand-50/80 border border-brand-100 p-3.5">
                  <p className="flex items-center gap-1.5 text-[11px] font-semibold text-brand-600 uppercase tracking-wide mb-2">
                    <FileSearch size={13} />
                    Số liệu chứng minh
                  </p>
                  <ul className="space-y-1.5">
                    {b.evidence.map((e) => (
                      <li key={e} className="text-xs text-slate-600 flex gap-2 leading-relaxed">
            ...
            className="rounded-2xl overflow-hidden shadow-md relative group h-64"
          >
            <img
              src={photo}
        ...
        <div className="flex flex-col gap-4">
          <motion.div
            ...
            className="rounded-2xl border border-brand-100 bg-white p-4 shadow-sm flex-1"
          >
